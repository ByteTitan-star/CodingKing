...
from coderking_coding_agent.tools.dynamic_adapter import wrap_dynamic_tools
from coderking_coding_agent.tools.dynamic_runner import SandboxToolRunner
from coderking_coding_agent.tools.test import RunTestsTool
...
class AgentRuntime:
    ...
    async def run(
        ...
        *,
        ...
        tools = dict(self.bindings.build_tools(workspace, sandbox))
        dynamic_loader = DynamicToolLoader(
            source,
            SandboxToolRunner(sandbox, timeout_sec=self.config.sandbox_timeout_sec),
        ...
        if test_command:
            tools["run_tests"] = RunTestsTool(
                sandbox, self.config.sandbox_timeout_sec, default_command=test_command
            )
        if not state.messages:
            state.messages = [
                {
                    "role": "system",
                    "content": self.bindings.resolve_system_prompt(state.role),
                },
            ...
            state.messages.append(
                {"role": "system", "content": self.bindings.resolve_system_prompt(Role.PLANNER)}
            )
        ...
        try:
            mcp_host = (
                await self.bindings.connect_mcp(source) if self.bindings.connect_mcp else None
            )
            ...
            while state.iteration < self.config.max_iterations:
                ...
                if (
                    ...
                    if (
                        cow is not None
                        and self.config.sandbox_rollback_on_interrupt
                        and state.snapshot
                    ):
            ...
            state.status = TaskStatus.INTERRUPTED
            if cow is not None and self.config.sandbox_rollback_on_interrupt and state.snapshot:
                restore_snapshot(workspace, state.snapshot)
            await on_event(done_event(False, "interrupted"))
            return state
